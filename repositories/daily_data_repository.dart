...
class DailyDataRepository {
    ...
    if (kIsWeb) {
      ...
      try {
        return _webData.firstWhere(
          (item) => item['timestamp_daily'] == timestamp,
        );
      } catch (e) {
        return null;
      }
  ...
  Future<int> updateData(Map<String, dynamic> data) async {
    if (kIsWeb) {
      // Web平台處理
      final timestamp = data['timestamp_daily'];
    ...
    if (_db == null) throw Exception('Database not initialized');
    // 用 timestamp_daily 當更新條件
    return await _db!.update(
      'daily_data',
      data,
      where: 'timestamp_daily = ?',
      whereArgs: [data['timestamp_daily']],
    );
  }
  ...
  Future<int> deleteByTimestamp(String timestamp) async {
    if (kIsWeb) {
      ...
      _webData.removeWhere((item) => item['timestamp_daily'] == timestamp);
  ...
  Future<List<String>> getAllTimestamps() async {
    if (kIsWeb) {
      // Web平台處理
      return _webData
          .map((item) => item['timestamp_daily'].toString())
          .toList();
    }
    ...
    final result = await _db!.query(
      'daily_data',
      columns: ['timestamp_daily'],
    );
    return result.map((row) => row['timestamp_daily'].toString()).toList();
  }
