...
class SheetsRepository {
  ...
  /// 例如: [
  ///   ['timestamp_daily', 'category', 'details', 'aed', 'usdt', 'cny', 'online', 'edit_note', 'last_modified'],
  ...
  Future<void> appendSingleRow(List<Object?> row) async {
    ...
    if (existingRows.isEmpty) {
      // 如果沒有數據，先添加表頭
      final header = [
        'timestamp_daily', 'category', 'details', 'aed', 
        'usdt', 'cny', 'online', 'edit_note', 'last_modified'
      ];
      await writeDataToSheets([header, row]);
      return;
    }
  ...
  Future<int?> findRowByTimestamp(String timestamp) async {
    ...
    for (int i = 1; i < values.length; i++) {
      if (values[i].isNotEmpty && values[i][0].toString() == timestamp) {
        return i + 1; // 返回 1-based 行號
      }
    }
    return null;
  }
